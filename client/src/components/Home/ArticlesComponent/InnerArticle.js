...
class InnerArticle extends React.Component{
    ...
    async getPosts(){
        const res = await axios.get(`http://localhost:3000/posts/${this.props.match.params.id}`);
        console.log(res)
    ...
    renderList(){
        ...
        <div className="ArticleBody">
            <h4 className="head-change-4">{this.state.post.title}</h4>
            ...
            <img className="react-img" src={this.state.post.image}></img>
            <p className="Article-para">{this.state.post.description}</p>
            <img className="code-img" src={this.state.post.secondImg}></img>
